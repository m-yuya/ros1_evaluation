...
private:
  virtual void onInit()
  {
    ...
    chatter_pub = private_nh.advertise<std_msgs::String>("/chatter", 10);

	ros::Rate loop_rate(PUBLISH_Hz);

	//	std::stringstream tmp;
	// tmp << "Hello World";
	// boost::shared_ptr<std_msgs::String> hello_ptr(new std_msgs::String);
	// hello_ptr->data = tmp.str();

	// while (ros::ok()) {
	//   printf("talker : %p \n", hello_ptr.get());
	//   chatter_pub.publish(hello_ptr);
	//   loop_rate.sleep();
	// }

	printf("Launched nodelet_eval/Talker \n");
...
PLUGINLIB_DECLARE_CLASS(nodelet_eval, Talker, nodelet_eval::Talker, nodelet::Nodelet);
}
